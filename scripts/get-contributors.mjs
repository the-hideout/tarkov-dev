...
(async () => {
  ...
  try {
    ...
    const totalRepContributors = allRepContributors.reduce(
    ...
    allContributors = Object.entries(totalRepContributors)
      .map(([login, totalContributions]) => {
        const { html_url, avatar_url } = allRepContributors.find(
  ...
  if (allContributors.length === 0) {
    ...
    try {
      ...
      console.warn(
        `Unable to read existing contributors.json fallback: ${readError}`,
      );
    }
