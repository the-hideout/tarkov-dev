...
/*import fleaMarketFee from '../src/modules/flea-market-fee.mjs';
import camelcaseToDashes from '../src/modules/camelcase-to-dashes.js';
...
const doFetchItems = async () => {
    ...
    const itemData = await fetch('https://api.tarkov.dev/graphql', {
        method: 'POST',
        headers: {
    ...
    const itemGrids = JSON.parse(await fs.readFile('./src/data/item-grids.json'));
    console.log(itemGrids);
