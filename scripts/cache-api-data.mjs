...
async function getLanguageCodes() {
    ...
    return graphqlRequest(query).then((response) =>
        response.data.__type.enumValues.map((lang) => {
            return lang.name;
        }),
    );
}
...
try {
    ...
    apiPromises.push(
        Promise.all([
            ...
            return doFetchItems({ prebuild: true }).then((itemsResults) => {
                ...
                const filteredAmmoDic = Object.values(groupedAmmoDic).map((group) =>
                    group.sort((a, b) => b.properties.damage - a.properties.damage).slice(0, 2),
                );
                ...
                fs.writeFileSync(
                    ...
                    JSON.stringify({ items: filteredItems, handbook: itemsResults.handbook }),
                );
    ...
    apiPromises.push(
        doFetchHideout({ prebuild: true }).then((hideout) => {
            fs.writeFileSync("./src/data/hideout_cached.json", JSON.stringify(hideout));
        }),
    );

    apiPromises.push(
        doFetchTraders({ prebuild: true }).then((traders) => {
    ...
    apiPromises.push(
        doFetchMaps({ prebuild: true }).then((maps) => {
            maps = maps.map((map) => {
                return {
                    ...map,
                    extracts: [],
    ...
    apiPromises.push(
        doFetchBosses({ prebuild: true }).then((bosses) => {
    ...
    apiPromises.push(
        doFetchQuests({ prebuild: true }).then((questsResult) => {
