...
import { Filter, ButtonGroupFilter, ButtonGroupFilterButton, ToggleFilter } from "../../components/filter/index.jsx";
        ...
        return hideout
            .map((station) => {
                return {
                    ...station,
                    levels:
                        (showBuilt && showLocked) || !settings.useTarkovTracker
                            ? station.levels
                            : station.levels.filter((lvl) => {
    ...
    return [
        ...
        <div className="page-wrapper" key={"display-wrapper"}>
            ...
            <Filter center>
                ...
                {settings.useTarkovTracker && (
                    <ToggleFilter
                        checked={showBuilt}
                        label={t("Show built")}
                        onChange={(e) => setShowBuilt(!showBuilt)}
                        tooltipContent={<>{t("Show already built stations")}</>}
                    />
                )}
                {settings.useTarkovTracker && (
                    <ToggleFilter
                        checked={showLocked}
                        label={t("Show locked")}
                        onChange={(e) => setShowLocked(!showLocked)}
                        tooltipContent={<>{t("Show unavailable stations")}</>}
                    />
                )}
                <ToggleFilter
                    checked={showTraderStationReqs}
                    label={t("Show all requirements")}
                    onChange={(e) => setShowTraderStationReqs(!showTraderStationReqs)}
                    tooltipContent={<>{t("Show trader and other station level requirements")}</>}
                />
            </Filter>
            {stations.map((hideoutModule) => {
                ...
                if (selectedStation && selectedStation !== "all" && hideoutModule.normalizedName !== selectedStation) {
                    return null;
                }
                    ...
                    return (
                        ...
                        >
                            ...
                            <ItemsSummaryTable
                                ...
                                includeStations={
                                    showTraderStationReqs
                                        ? level.stationLevelRequirements.filter(
                                              (req) => req.station.id !== hideoutModule.id,
                                          )
                                        : []
                                }
                            />
