...
const ServerStatus = lazy(
  () => import("../../components/server-status/index.jsx"),
);
const SmallItemTable = lazy(
  () => import("../../components/small-item-table/index.jsx"),
);
...
function Start() {
  ...
  const uniqueMaps = mapImagesSortedArray.reduce((maps, current) => {
    if (
      !maps.some(
        (storedMap) => storedMap.normalizedName === current.normalizedName,
      )
    ) {
      maps.push({
        name: current.name,
