    ...
    const itemsQuery = useQuery(
        ...
        async () => {
            ...
            const allItems = itemData.data.itemsByType.map((rawItem) => {
                ...
                rawItem.itemProperties =
                    itemProps[rawItem.id]?.itemProperties || {};
                ...
                if (itemProps[rawItem.id]?.hasGrid) {
                    ...
                    if (gridPockets.length > 1) {
                        grid.height = Math.max(
                            ...gridPockets.map(
                                (pocket) => pocket.row + pocket.height,
                            ),
                        );
                        grid.width = Math.max(
                            ...gridPockets.map(
                                (pocket) => pocket.col + pocket.width,
                            ),
                        );
                    }
                    ...
                    if (
                        !itemGrids[rawItem.id] &&
                        !rawItem.types.includes('backpack')
                    ) {
                        grid = false;
                    }
                }
            ...
            for (const item of allItems) {
                if (item.types.includes('gun') && item.containsItems) {
                    item.traderPrices = item.traderPrices.map(
                        (localTraderPrice) => {
                            ...
                            localTraderPrice.price = item.containsItems.reduce(
                                (previousValue, currentValue) => {
                                    const partPrice = itemMap[
                                        ...
                                        (innerTraderPrice) =>
                                            innerTraderPrice.name ===
                                            localTraderPrice.name,
                                    );
