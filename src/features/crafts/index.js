...
export const selectAllCrafts = createSelector([selectCrafts, selectQuests, selectItems], (crafts, quests, items) => {
    const validatedCrafts = crafts
                ...
                requiredItems: craft.requiredItems
                    .map((req) => {
                        ...
                        if (matchedItem && matchedItem.types.includes("gun")) {
                            if (
                                req.attributes?.some(
                                    (element) => element.type === "functional" && Boolean(element.value),
                                )
                            ) {
        ...
        .filter(
            (craft) =>
                craft.rewardItems.length > 0 && (craft.requiredItems.length > 0 || craft.startRequiredItemCount === 0),
        );
    validatedCrafts.forEach((craft) => {
        delete craft.startRequiredItemCount;
    });
