...
import { mdiCogOutline, mdiRemote, mdiClose } from "@mdi/js";
...
const Menu = () => {
  ...
  const [alertsClosed, setAlertsClosed] = useStateWithLocalStorage(
    "alertBannersClosed",
    [],
  );
        ...
        <ul className={`menu`}>
          <IntersectionObserverWrapper>
            <li
              key="menu-home"
              data-targetid="home"
              className="overflow-member"
            >
              <Badge
                badgeContent={
                  loadingData ? <LinearProgress /> : gameModeTranslated
                }
                color={gameModeBadgeColor}
            ...
            <li
              className="submenu-wrapper overflow-member"
              key="menu-settings"
            ...
            <li
              className="submenu-wrapper submenu-items overflow-member"
              key="menu-maps"
              data-targetid="maps"
            >
              ...
              <ul style={{ left: -40 }}>
                {Object.values(
                  uniqueMaps.reduce((unique, map) => {
                    const sameMap = Object.values(unique).find(
                      (m) => m.id === map.id,
                    );
                    if (!sameMap) {
                      unique[map.id] = map;
            ...
            <li
              className="submenu-wrapper submenu-items overflow-member"
              key="menu-achievements"
              data-targetid="achievements"
            >
              <Link
                to="/achievements"
                //onClick={setIsOpen.bind(this, false)}
            ...
            <li
              className="submenu-wrapper submenu-items overflow-member"
              key="menu-tarkov-monitor"
              data-targetid="tarkov-monitor"
            >
              <Link to="/tarkov-monitor">{t("TarkovMonitor")}</Link>
            </li>
            <li
              className="submenu-wrapper submenu-items overflow-member"
              key="menu-stash"
