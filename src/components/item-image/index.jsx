...
function ItemImage({
  ...
  const backgroundStyle = useMemo(() => {
    ...
    let height =
      imageDimensions.height || (itemSize.height * 63 + 1) * sizeFactor;
    const gridSvg = () => (
      ...
      <div style={itemExtraStyle}>
        ...
        {count && (
          <span className="item-image-count">
            {trader ? t("LL{{level}}", { level: count }) : count}
          </span>
        )}
      </div>
      {trader && (
        <div style={traderElementStyle}>
          <Tooltip placement="top" title={trader.name} arrow>
            <Link
              to={`/trader/${trader.normalizedName}/?tab=${count}&search=${item.name}`}
            >
              <img
                alt={trader.name}
