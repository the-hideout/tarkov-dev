...
L.Control.MapSettings = L.Control.extend({
  ...
  onAdd: function (map) {
    ...
    var link = (this._settingsLink = L.DomUtil.create(
      "a",
      className + "-toggle",
      container,
    ));
    ...
    const expandMapLegendDiv = L.DomUtil.create(
      "div",
      `${className}-setting-container`,
      form,
    );

    const expandMapLegendLabel = L.DomUtil.create(
      "label",
      undefined,
      expandMapLegendDiv,
    );
    ...
    const expandMapLegendCheckbox = L.DomUtil.create(
      "input",
      undefined,
      expandMapLegendLabel,
    );
    ...
    if (!!this.options.expandMapLegendChecked) {
      expandMapLegendCheckbox.setAttribute(
        "checked",
        !!this.options.expandMapLegendChecked,
      );
      expandMapLegendCheckbox.checked = true;
    }
    L.DomEvent.on(
      expandMapLegendCheckbox,
      "click",
      this._onSettingChanged,
      this,
    );

    const expandMapLegendLabelContent = L.DomUtil.create(
      "span",
      undefined,
      expandMapLegendLabel,
    );
    ...
    const expandSearchDiv = L.DomUtil.create(
      "div",
      `${className}-setting-container`,
      form,
    );

    const expandSearchLabel = L.DomUtil.create(
      "label",
      undefined,
      expandSearchDiv,
    );
    ...
    const expandSearchCheckbox = L.DomUtil.create(
      "input",
      undefined,
      expandSearchLabel,
    );
    ...
    if (!!this.options.expandSearchChecked) {
      expandSearchCheckbox.setAttribute(
        "checked",
        !!this.options.expandSearchChecked,
      );
      expandSearchCheckbox.checked = true;
    }
    L.DomEvent.on(expandSearchCheckbox, "click", this._onSettingChanged, this);

    const expandSearchLabelContent = L.DomUtil.create(
      "span",
      undefined,
      expandSearchLabel,
    );
    ...
    const activeQuestMarkersDiv = L.DomUtil.create(
      "div",
      `${className}-setting-container`,
      form,
    );

    const activeQuestMarkersLabel = L.DomUtil.create(
      "label",
      undefined,
      activeQuestMarkersDiv,
    );
    activeQuestMarkersLabel.setAttribute("for", "showOnlyActiveTasks");

    const activeQuestMarkersCheckbox = L.DomUtil.create(
      "input",
      undefined,
      activeQuestMarkersLabel,
    );
    ...
    if (!!this.options.activeTasksChecked) {
      activeQuestMarkersCheckbox.setAttribute(
        "checked",
        !!this.options.activeTasksChecked,
      );
      activeQuestMarkersCheckbox.checked = true;
    }
    L.DomEvent.on(
      activeQuestMarkersCheckbox,
      "click",
      this._onSettingChanged,
      this,
    );

    const activeQuestMarkersLabelContent = L.DomUtil.create(
      "span",
      undefined,
      activeQuestMarkersLabel,
    );
    ...
    L.DomUtil.create(
      "div",
      `${className}-separator player-location-help-separator`,
      form,
    );

    // show location labels setting
    const playerLocationDiv = L.DomUtil.create(
      "div",
      `${className}-player-location-help`,
      form,
    );
    const playerLocationLink = L.DomUtil.create(
      "a",
      undefined,
      playerLocationDiv,
    );
    playerLocationLink.setAttribute(
      "href",
      "/tarkov-monitor#website-integration",
    );
    playerLocationLink.setAttribute("target", "_blank");
    playerLocationLink.textContent =
      this.options.playerLocationLabel ??
      "Use TarkovMonitor to show your position";
